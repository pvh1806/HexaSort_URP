 ...
 */
    public class SecurePlayerPrefs : ScriptableObject
    {
        ...
        { 10, 20, 30, 40, 50, 60, 70, 80, 90, 12, 13, 14, 15, 16, 17, 18, 19 };
        private static byte[] keyArray = Encoding.UTF8.GetBytes("H2h2xZe83AX90788QNqJXRiWX88xWI2b"); // 256-AES key Replace with your own
        ...
        public static int GetInt(string Key, int v = 0)
        {
            String t = PlayerPrefs.GetString(Encrypt(Key));
            if (t == "") return v;
            return int.Parse(Decrypt(t));
        }
        public static float GetFloat(string Key, int v = 0)
        {
            String t = PlayerPrefs.GetString(Encrypt(Key));
            if (t == "") return v;
